...
function EmailCheck(Email){
	if(Email==""){
			...
			var mail=$("mail");
			alert(mail); // [object ~~]
			$("e_html").innerHTML="<font color='red'>이메일을 먼저 입력하여주세요.</font>";
			$("mail").focus();
			return false;
	}
	//입력했다면.Ajax를 이용해서 IdCheck.jsp를 요청
	getXHR();
	var url="http://localhost:8080/TodayLunch/Email.jsp?"+getEmailValues();
	alert(url);
	//2.콜백함수 지정
	xhrObject.onreadystatechange=Emailsubmit;
	//3. open함수를 이용해서 서버에 요청준비
	xhrObject.open("GET",url,true); // 비동기
	//4. send() 함수를 이용해서 요청
	xhrObject.send(null);
...
function getEmailValues(){
	var mail=$("mail").value; // document.RegForm.mem_id.value
	
	return "to="+mail+"&timestamp="+new Date().getTime();
	
}


function Emailsubmit(){ // xml or text(html태그를 포함된 문자열)로 콜백된다.
	...
	if(xhrObject.readyState==4){//서버에게 요청이 됐다면.
		...
		if(xhrObject.status==200){// 서버의 결과를 받았다면.
				var result=xhrObject.responseText; // 태그 + 문자열
				alert('이거나오냐email');
					$("e_html").innerHTML="인증번호:<input type='text' size='10'> <input type='button' value='인증번호 확인'"
			}
		}
	}
